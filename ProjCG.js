...
function checkCollisions(obj){
    ...
    if (!isTruck) {
        return; // Collision detection only in truck mode
    }

    const truckAABB = robot.getAABB(); // Get AABB of the truck
    const trailerAABB = trailer.getAABB(); // Get AABB of the trailer
...
function handleMovement(delta) {
    ...
    if (keyRHolded && head.userData.nTimes < foldingSpeed) { //key R/r //CLOCK
        // fold head
        /*
...
function animate() {
    ...
    if (collisionDetectionActive) {
        ...
        detectCollision(truck, trailer, isTruck);
    }
...
function onKeyDown(e) {
    ...
    if (cameraInputs.includes(e.keyCode)) { //keys 1 to 5
        if (!collisionDetectionActive) {
            collisionDetectionActive = true; // Re-enable collision detection
        }
